...
**Step 1: Stop any stale processes on ports 8001 and 3000**
```powershell
$p = Get-NetTCPConnection -LocalPort 8001 -ErrorAction SilentlyContinue; if ($p) { Stop-Process -Id $p[0].OwningProcess -Force -ErrorAction SilentlyContinue }
...
This starts ALL services: Redis, TimescaleDB, Backend API (port 8001), Scraper (LotusBook polling), Orchestrator (autonomous training), and Frontend (port 3000).
